...
$(document).ready(function(){
  ...
  $('.nav-contain').meanmenu({
    removeElements: ".power-last ul:nth-of-type(2), .bunny, .batteries",
  });
    ...
    //// Find filter buttons
	////change on 4/8/15 to disable filter option
    //var $filters = $('.filter-area').find('li.filter');
	////End change
    ...
    $filters.on('click', function () {
        ...
        $current = $('.filter-area').find('li.filter.active');
        console.log($current.length + ' is how many active AFTER');
        if($current.length == 0){
                $('.filtered-products-container').mixItUp('filter', 'all');
        }
